...
export default function FileUpload({ files, onFilesChange, hasVision = true }: FileUploadProps) {
      ...
      {files.length > 0 && (
        <div className="mt-4 space-y-2">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm text-slate-400">{files.length} file{files.length > 1 ? 's' : ''} selected</span>
            <button
              onClick={clearAll}
          ...
          {files.map((file, index) => (
            <div key={`${file.name}-${index}`} className="flex items-center justify-between p-3 bg-slate-800 border border-slate-700 rounded-lg">
              ...
              <button
                ...
                className="p-1.5 hover:bg-red-900/20 rounded-lg border border-transparent hover:border-red-500/50 transition-all group flex-shrink-0 ml-2"
